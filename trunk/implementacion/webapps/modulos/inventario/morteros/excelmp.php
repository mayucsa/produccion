...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', 'INVENTARIO DE MATERIA PRIMA');
...
// $objPHPExcel->setActiveSheetIndex(0)->getStyle('H3:I3')->applyFromArray($encabezado);
	...
	$datos = $objModelo->mostrarExcelMatPrima();
	$ite = 4;

	foreach ($datos as $clave) {
            $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A'.$ite, $clave['nombre_materia_prima'])        
            ->setCellValue('B'.$ite, $clave['cantidad_materia_prima'])
...
header('Content-Disposition: attachment;filename="Inventario Materia Prima.xls"');
