function consultar(){
    ...
    $('#tablaConfirmacion').DataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": "serverSideconfirmacion.php",
        ...
        "columnDefs":[
                        ...
                        },
                        {
                            ...
                            "render": function(data, type, row, meta){
                                ...
                                // "data": 'cve_entrada',
                                return '<span class= "btn btn-success" onclick= "obtenerDatos('+row[7]+')" data-toggle="modal" data-target="#modalConfirmacion"><i class="fas fa-check-circle"></i> </span>';
                            }
                            // "data": null,
        ...
        "columnDefs":[
                        ...
                        },
                        {
                            ...
                            "render": function(data, type, row, meta){
                                // const primaryKey = data;
                                // "data": 'cve_entrada',
                                return '<span class= "btn btn-success" onclick= "sinacceso()"><i class="fas fa-check-circle"></i> </span>';
                            }
                            // "data": null,
                            // "defaultContent": '<span class= "btn btn-warning" onclick= "obtenerDatos(".$value["cve_entrada"].")" data-toggle="modal" data-target="#modalMatPrimaUpdate" data-whatever="@getbootstrap"><i class="fas fa-edit"></i> </span>'
                        }
...
function limpiarCampos() {
    $('#comb_areac').val("0");
    $('#comb_productoc').val("0");
    $('#comb_presentc').val("0");
    $('#comb_celdasc').val("");
    $('#input_canttotalc').val("");
    $('#input_cantdespuntadosc').val("");
    $('#input_estiba').val("");
}
...
function obtenerDatos(cve_desalojo) {
    $.getJSON("modelo_confirmacion.php?consultar="+cve_desalojo, function(registros){
        // console.log(registros);

        $('#comb_idc').val(registros[0]['cve_desalojo']);
        $('#comb_areac').val(registros[0]['area']);
        $('#comb_productoc').val(registros[0]['nombre_producto']);
        $('#comb_presentc').val(registros[0]['presentacion']);
        $('#comb_celdasc').val(registros[0]['num_celdas']);
        ...
        $('#input_cantrotos').val(registros[0]['cantidad_rotura']);
    });
}
...
function cerrarModal() {
    ...
    // $('#cerrarmodalconfirmacion').click();
    // alert("Cerrar");
}

function confirmacionDesalojo(cve_desalojo) {
    // var select = $('#comb_quimico').val();
    var despuntados = $('#input_estiba').val();
    var msj = "";
  
    if (despuntados == "") {
        msj += 'Número de estiba <br>';
    }
    ...
    datos.append('cve_desalojo',    $('#comb_idc').val());
    datos.append('nombre_producto', $('#comb_productoc').val());
    datos.append('area',            $('#comb_areac').val());
    datos.append('presentacion',    $('#comb_presentc').val());
    datos.append('celdas',          $('#comb_celdasc').val());
    datos.append('cantidad',        $('#input_canttotalc').val());
    datos.append('despuntados',     $('#input_cantdespuntadosc').val());
    datos.append('estiba',          $('#input_estiba').val());
    datos.append('user',            $('#spanuser').text());

    // console.log(datos.get('cve_desalojo'));
    ...
    // console.log(datos.get('user'));
        Swal.fire({
                ...
                html: 'Area: <b>' + datos.get('area') + 
                '</b><br> Producto: <b>'+ datos.get('nombre_producto') + ' - ' + datos.get('presentacion') + ' - ' + datos.get('celdas') + ' celdas' +
                '</b><br> Cantidad desalojo: <b>' + datos.get('cantidad') +
    ...
    if (result.isConfirmed) {
        $.ajax({
                type:"POST",
                url:"modelo_confirmacion.php?actualizar=1",
                data: datos,
                processData:false,
                contentType:false,
        success:function(r){
...
function RoturaDiaria(cve_estiba) {
    ...
    // console.log(datos.get('user'));
        Swal.fire({
                ...
                '</b><br> Cantidad de rotura: <b>' + datos.get('cantidad'),
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
