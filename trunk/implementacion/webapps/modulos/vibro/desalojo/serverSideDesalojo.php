...
$var    = "area = 'VibroBlock' AND  estatus_desalojo = 'VIG'";
$columns = array(
    array( 'db' => 'nombre_producto',   'dt' => 0 ),
    array( 'db' => 'presentacion',      'dt' => 1 ),
    array( 'db' => 'cantidad_total',          'dt' => 2,
                    'formatter' => function( $d, $row ) {
            return number_format($d).' Pza';
        } ),
    array( 'db' => 'cantidad_rotura',          'dt' => 3,
                    'formatter' => function( $d, $row ) {
            return number_format($d).' Pza';
        } ),
    array( 'db' => 'cantidad_despuntados',          'dt' => 4,
                    'formatter' => function( $d, $row ) {
            return number_format($d).' Pza';
        } ),
    array(
        'db'        => 'fecha_registro', 'dt'        => 5
    ),
